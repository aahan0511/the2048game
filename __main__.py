...
from os import getlogin, makedirs, system
...
USER = getlogin()
ASSET_DIRECTORY = f"."
DB_DIRECTORY = rf"C:\Users\{USER}\AppData\the2048game"
COLORS = [
    None,
    "#eee4da",
    "#ebd8b6",
...
PATHS = {
    "empty": f"{ASSET_DIRECTORY}\\assets\\images\\empty.ico",
    "icon": f"{ASSET_DIRECTORY}\\assets\\images\\icon.png",
    "JetBrainsMono-Medium.ttf": ASSET_DIRECTORY
    + "\\assets\\fonts\\JetBrainsMono-Medium.ttf",
    "JetBrainsMono-Bold.ttf": ASSET_DIRECTORY
    + "\\assets\\fonts\\JetBrainsMono-Bold.ttf",
    "how to play.png": ASSET_DIRECTORY + "\\assets\\images\\how to play.png",
    "info.png": ASSET_DIRECTORY + "\\assets\\images\\info.png",
    "license.png": ASSET_DIRECTORY + "\\assets\\images\\license.png",
}
SPEED = 5
GROW_SPEED = 3.6
TABLE_LEN = 20

SCREEN_FACTOR = 0.9  # Recommended 0.75 to 1
...
conn = connect(f"{DB_DIRECTORY}\\scores.db")
cursor = conn.cursor()

cursor.execute(
    "CREATE TABLE IF NOT EXISTS Scores (Time TEXT, Score INTEGER, Block INTEGER, NAME TEXT)"
)
...
class App(CTk):
    def __init__(root) -> None:
        ...
        root.geometry(
            f"{int(715 * SCREEN_FACTOR)}x{int(700 * SCREEN_FACTOR)}+{int(50 * SCREEN_FACTOR)}+{int(50 * SCREEN_FACTOR)}"
        )
        ...
        change_title_color(root, "#faf8f0")
        ...
        root.highVar = IntVar(
            value=cursor.execute(f"""SELECT MAX(Score) FROM Scores""").fetchone()[0]
        )
        icon = ImageTk.PhotoImage(
            Image.open(PATHS["icon"]).resize(
                (int(80 * SCREEN_FACTOR), int(80 * SCREEN_FACTOR))
            )
        )

        root.more = More(master=root)
        root.header = Header(
            master=root, scoreVar=root.scoreVar, highVar=root.highVar, icon=icon
        )
        ...
        if not pause:
            ...
            if ongoing:
                ...
                if root.scoreVar.get() != 0:
                    ...
                    cursor.execute(
                        f"INSERT INTO Scores VALUES ('{datetime.now().date()} {datetime.now().hour}:{datetime.now().minute}.{datetime.now().second}', {root.scoreVar.get()}, {2 ** max(matrix)}, '{name.get_input()}')"
                    )
                    conn.commit()
                for cell in grid:
                    if cell:
                        cell.destroy()
                highscore = cursor.execute(f"SELECT MAX(Score) FROM Scores").fetchone()[
                    0
                ]
...
class More(CTkFrame):
    def __init__(more, master: App) -> None:
        ...
        more.leaderboard = Leaderboard(
            more, more.leaderboardFilter, more.leaderboardSize
        )
...
class Leaderboard(CTkTabview):
    def __init__(
        board, master: More, boardFilter: StringVar, boardSize: StringVar
    ) -> None:
        super().__init__(
            master,
            fg_color="#9b8878",
    ...
    def refresh(board, need=False, *_) -> None:
        ...
        if filtr == 0:
            ...
            values = sorted(
                cursor.execute(f"SELECT * FROM Scores").fetchall(),
                key=lambda item: toDate(item),
                reverse=(True if filtr in [0, 3] else False),
            )
        else:
            values = sorted(
                cursor.execute(f"SELECT * FROM Scores").fetchall(),
                key=lambda item: item[filtr],
                reverse=(True if filtr != 3 else False),
            )

        if (
            board.values != values or board.values is None or need
        ) and board.size.get() != "":
            try:
            ...
            for tabNum in range(board.length):
                ...
                CTkTable(
                    ...
                    column=4,
                    values=[("TIME", "SCORE", "BLOCK", "NAME")]
                    + values[size * tabNum : size * (tabNum + 1)],
                    corner_radius=35,
                    header_color="#bdac97",
                    text_color="#756452",
...
class Header(CTkFrame):
    def __init__(
        header, master: App, scoreVar: IntVar, highVar: IntVar, icon: ImageTk.PhotoImage
    ) -> None:
        super().__init__(
            master,
            fg_color="#bdac97",
        ...
        header.curScore = Score(
            master=header, color="#eae7d9", pos=1, description="SCORE", var=scoreVar
        )

        header.highScore = Score(
            master=header, color="#faf8f0", pos=2, description="BEST", var=highVar
        )
...
class NamePlate(CTkFrame):
    def __init__(nameplate, master: Header, icon: ImageTk.PhotoImage) -> None:
        ...
        nameplate.logo.create_image(
            40 * SCREEN_FACTOR, 40 * SCREEN_FACTOR, image=icon, anchor="center"
        )
        ...
        nameplate.grid(
            ...
            column=0,
            sticky="nsew",
            padx=10 * SCREEN_FACTOR,
            pady=19 * SCREEN_FACTOR,
        )


class Score(CTkFrame):
    def __init__(
        score, master: Header, color: str, pos: int, description: str, var: IntVar
    ) -> None:
        super().__init__(
            master,
            corner_radius=25,
        ...
        score.label = CTkLabel(
            score,
            text=description,
            font=("JetBrains Mono Medium", 13 * SCREEN_FACTOR),
            text_color="#988a86",
        )
...
class Side(CTkFrame):
    def __init__(side, master: App, more: More) -> None:
        ...
        side.moreButton.grid(
            column=0,
            row=0,
            sticky="nsew",
            padx=19 * SCREEN_FACTOR,
            pady=19 * SCREEN_FACTOR,
        )
        side.more = more
...
class GameScreen(CTkFrame):
    def __init__(screen, master: App) -> None:
        ...
        screen.fontsizes = {
            1: int(28 * SCREEN_FACTOR),
            2: int(26 * SCREEN_FACTOR),
            3: int(22 * SCREEN_FACTOR),
            4: int(18 * SCREEN_FACTOR),
        }
...
class Info(CTkToplevel):
    def __init__(top, master: App) -> None:
        ...
        top.after(
            200,
            lambda: top.bind(
                "<Button>", lambda _: openWeb("https://github.com/aahan0511")
            ),
        )
...
class Notification(CTkFrame):
    def __init__(notifier, master: GameScreen, version: bool) -> None:
        if version:
            super().__init__(master, fg_color="#ffd700", corner_radius=35)
            ...
            super().__init__(master, fg_color="#888888", corner_radius=35)
            notifier.grid(row=0, column=0, columnspan=4, rowspan=4, sticky="nsew")
            notifier.lift()
...
class Block:
    def __init__(block, master: GameScreen) -> None:
        ...
        block.cell = CTkLabel(
            ...
            font=(
                "JetBrains Mono Bold",
                28 * SCREEN_FACTOR if block.power <= 13 else 24 * SCREEN_FACTOR,
            ),
            justify="center",
            anchor="center",
            width=116.25 * SCREEN_FACTOR,
    ...
    def place(block) -> None:
        block.cell.place(x=block.x, y=block.y, anchor="center")

        def grow(side):
            block.cell.configure(
                width=116.25 * SCREEN_FACTOR - side,
                height=116.25 * SCREEN_FACTOR - side,
                justify="center",
                anchor="center",
                corner_radius=35,
            )
            if side > 0:
                block.cell.after(
                    1, lambda: grow(side - GROW_SPEED if side >= GROW_SPEED else 0)
                )
                block.cell.lift()
                if showingLoss:
    ...
    def set(block) -> None:
        ...
        block.cell.configure(
            ...
            text_color="#ffffff" if block.power > 2 else "#756452",
            font=CTkFont(
                "JetBrains Mono Bold", block.master.fontsizes[len(str(block.var.get()))]
            ),
        )
        grid[block.pos] = block
        matrix[block.pos] = block.power
...
if app.scoreVar.get() != 0:
    ...
    cursor.execute(
        f"INSERT INTO Scores VALUES ('{datetime.now().date()} {datetime.now().hour}:{datetime.now().minute}.{datetime.now().second}', {app.scoreVar.get()}, {2 ** max(matrix)}, '{name.get_input()}')"
    )
    conn.commit()
